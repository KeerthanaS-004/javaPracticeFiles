...
public class variableKnapsackuh {
    public static void main(String[] args) {
        ...
        double remainingCapacity = capacity;//2

        System.out.println("\nItems selected:");
        for (int i = 0; i < n; i++) {//2<2
            if (remainingCapacity == 0) {//1 == 0
                break;
            }
            if (weight[i] <= remainingCapacity) {//4 <= 1
                // Take the whole item
                totalValue += value[i];//500
                ...
                double fraction = remainingCapacity / weight[i];//0
                totalValue += value[i] * fraction;//1000*0=0
